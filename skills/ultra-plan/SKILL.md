...
<Purpose>
Ultra Plan creates comprehensive, actionable work plans through intelligent interaction. It auto-detects whether to interview the user (broad requests) or plan directly (detailed requests), and supports consensus mode (iterative Planner/Architect/Critic loop with RALPLAN-DR structured deliberation) and review mode (Critic evaluation of existing plans).
</Purpose>
...
<Do_Not_Use_When>
- User wants autonomous end-to-end execution -- use `ultrapilot` instead
- User wants to start coding immediately with a clear task -- use `ultra-ralph` or delegate to ultra-executor
...
<Why_This_Exists>
Jumping into code without understanding requirements leads to rework, scope creep, and missed edge cases. Ultra Plan provides structured requirements gathering, expert analysis, and quality-gated plans so that execution starts from a solid foundation. The consensus mode adds multi-perspective validation for high-stakes projects.
</Why_This_Exists>

<Execution_Policy>
- Auto-detect interview vs direct mode based on request specificity
- Ask one question at a time during interviews -- never batch multiple questions
- Gather codebase facts via `ultra-explore` agent before asking the user about them
...
3. **Gather codebase facts first**: Before asking "what patterns does your code use?", delegate to ultra-explore agent to find out, then ask informed follow-up questions
4. **Build on answers**: Each question builds on the previous answer
5. **Consult ultra-analyst** (Opus) for hidden requirements, edge cases, and risks
6. **Create plan** when the user signals readiness: "create the plan", "I'm ready", "make it a work plan"

### Direct Mode (detailed requests)

1. **Quick Analysis**: Optional brief ultra-analyst consultation
2. **Create plan**: Generate comprehensive work plan immediately
3. **Review** (optional): ultra-critic review if requested
...
1. **ultra-planner** creates initial plan and a compact **RALPLAN-DR summary** before any Architect review. The summary **MUST** include:
   - **Principles** (3-5)
...
2. **User feedback** *(--interactive only)*: If running with `--interactive`, **MUST** use `AskUserQuestion` to present the draft plan **plus the RALPLAN-DR Principles / Decision Drivers / Options summary for early direction alignment** with these options:
   - **Proceed to review** — send to ultra-architect and ultra-critic for evaluation
   - **Request changes** — return to step 1 with user feedback incorporated
   - **Skip review** — go directly to final approval (step 7)
   If NOT running with `--interactive`, automatically proceed to review (step 3).
3. **ultra-architect** reviews for architectural soundness using `Task(subagent_type="ultra-verifier", ...)`. Architect review **MUST** include: strongest steelman counterargument (antithesis) against the favored option, at least one meaningful tradeoff tension, and (when possible) a synthesis path. In deliberate mode, Architect should explicitly flag principle violations. **Wait for this step to complete before proceeding to step 4.** Do NOT run steps 3 and 4 in parallel.
4. **ultra-critic** evaluates against quality criteria using `Task(subagent_type="ultra-code-reviewer", ...)`. Critic **MUST** verify principle-option consistency, fair alternative exploration, risk mitigation clarity, testable acceptance criteria, and concrete verification steps. Critic **MUST** explicitly reject shallow alternatives, driver contradictions, vague risks, or weak verification. In deliberate mode, Critic **MUST** reject missing/weak pre-mortem or missing/weak expanded test plan. Run only after step 3 is complete.
5. **Re-review loop** (max 5 iterations): If Critic rejects, execute this closed loop:
   a. Collect all rejection feedback from ultra-architect + ultra-critic
   b. Pass feedback to ultra-planner to produce a revised plan
   c. **Return to Step 3** — ultra-architect reviews the revised plan
   d. **Return to Step 4** — ultra-critic evaluates the revised plan
...
6. **Apply improvements**: When reviewers approve with improvement suggestions, merge all accepted improvements into the plan file before proceeding. Final consensus output **MUST** include an **ADR** section with: **Decision**, **Drivers**, **Alternatives considered**, **Why chosen**, **Consequences**, **Follow-ups**. Specifically:
   a. Collect all improvement suggestions from ultra-architect and ultra-critic responses
   b. Deduplicate and categorize the suggestions
   c. Update the plan file in `.ultra/plans/` with the accepted improvements (add missing details, refine steps, strengthen acceptance criteria, ADR updates, etc.)
...
7. On Critic approval (with improvements applied): *(--interactive only)* If running with `--interactive`, use `AskUserQuestion` to present the plan with these options:
   - **Approve and execute** — proceed to implementation via ultra-ralph+ultrawork
   - **Approve and implement via team** — proceed to implementation via coordinated parallel team agents
   - **Clear context and implement** — compact the context window first (recommended when context is large after planning), then start fresh implementation via ultra-ralph with the saved plan file
...
9. On user approval (--interactive only):
   - **Approve and execute**: **MUST** invoke `Skill("ultra-ralph")` with the approved plan path from `.ultra/plans/` as context. Do NOT implement directly. Do NOT edit source code files in the planning agent. The ultra-ralph skill handles execution via ultrawork parallel agents.
   - **Approve and implement via team**: **MUST** invoke `Skill("ultra-team")` with the approved plan path from `.ultra/plans/` as context. Do NOT implement directly. The ultra-team skill coordinates parallel agents across the staged pipeline for faster execution on large tasks.
   - **Clear context and implement**: First invoke context compaction, then invoke `Skill("ultra-ralph")` with the approved plan path from `.ultra/plans/`. This path is recommended when the context window is 50%+ full after the planning session.

### Review Mode (`--review`)

1. Read plan file from `.ultra/plans/`
2. Evaluate via ultra-critic using `Task(subagent_type="ultra-code-reviewer", ...)`
...
Plans are saved to `.ultra/plans/`. Drafts go to `.ultra/drafts/`.
</Steps>

<Tool_Usage>
- Use `AskUserQuestion` for preference questions (scope, priority, timeline, risk tolerance) -- provides clickable UI
- Use plain text for questions needing specific values (port numbers, names, follow-up clarifications)
- Use ultra-explore agent (Haiku, 30s timeout) to gather codebase facts before asking the user
- Use `Task(subagent_type="ultra-planner", ...)` for planning validation on large-scope plans
- Use `Task(subagent_type="ultra-debugger", ...)` for requirements analysis
- Use `Task(subagent_type="ultra-code-reviewer", ...)` for plan review in consensus and review modes
- **CRITICAL — Consensus mode agent calls MUST be sequential, never parallel.** Always await the ultra-architect Task result before issuing the ultra-critic Task.
...
- In consensus mode with `--interactive`, on user approval **MUST** invoke `Skill("ultra-ralph")` for execution (step 9) -- never implement directly in the planning agent
- When user selects "Clear context and implement" in step 7 (--interactive only): invoke context compaction first to compress the accumulated planning context, then immediately invoke `Skill("ultra-ralph")` with the plan path -- the compact step is critical to free up context before the implementation loop begins
</Tool_Usage>
...
```
Planner: [delegates to ultra-explore agent: "find authentication implementation"]
...
```
Why bad: The planner should delegate to ultra-explore agent to find this, not ask the user.
</Bad>
...
- In consensus mode, stop after 5 ultra-planner/ultra-architect/ultra-critic iterations and present the best version
...
- If the user says "just do it" or "skip planning", **MUST** invoke `Skill("ultra-ralph")` to transition to execution mode. Do NOT implement directly in the planning agent.
...
|------|----------|--------|
| Codebase Fact | "What patterns exist?", "Where is X?" | Delegate to ultra-explore first, do not ask user |
