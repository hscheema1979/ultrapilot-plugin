...
- `Task(subagent_type="ultra-executor", model="sonnet", ...)` for analytical tasks
- `Task(subagent_type="ultra-designer", model="sonnet", ...)` for design tasks

## Invocation

```
/ultra-ccg [task description]
```

Example:
```
/ultra-ccg Review this PR - check architecture and code quality (Codex) and UI components (Gemini)
```
